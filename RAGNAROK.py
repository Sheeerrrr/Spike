...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id):
    ...
    try:
        ...
        if selected_binary == "Smokie":
            ...
            command = f"./RAGNAROK {target_ip} {target_port} {duration}"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @SAYYED_Z*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @SAYYED_Z*", parse_mode='Markdown')
        return
    ...
    if is_super_admin(user_id):
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                ...
                "Contact @SAYYED_Z AND @Norze18 for assistance."
            )
        else:
            account_info = (
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @SAYYED_Z AND @Norze."
